...
//    ui->centerWidget->setGeometry(5,5,width,height);
    ...
    this->setAttribute(Qt::WA_TranslucentBackground,true);//窗口背景半透明
    //实例阴影shadow(三维阴影)
...
void Widget::Init()
{
    ...
    m_session = new Session(socket);//创建协议
    //绑定处理消息信号  和处理消息回调
    ...
    do{
        loginDlg.show();
        int status = loginDlg.exec();//设置模态窗口  阻塞
        if(status == QDialog::Accepted)//用户点击了登录
        {
            ChatLogInfo()<<u8"将要登录......";
            ...
            ChatLogInfo()<<u8"获得密码:"<<m_userInfo.m_password;
            login();//发送登录请求 接收登录响应
        }
...
int Widget::login()
{
    ...
    ChatLogInfo()<<u8"账号:"<<loginInfo.m_account<<u8",密码:"<<m_userInfo.m_password;
...
int Widget::handleLogin(void *msg)
{
    ...
    ChatLogInfo()<<u8"账号:"<<loginInfo->m_account<<u8",用户名:"<<loginInfo->m_userName;
...
//    short ret = RET_OK;
    ...
    ChatLogInfo()<<"处理消息:"<<packet->command<<",str:"<<getChatCmdString(packet->command);
    switch (packet->command)
    {
        case CommandEnum_Registe://注册
            // handleRegiste(rMsg->body + sizeof(DeMessagePacket));
            break;
        case CommandEnum_Login://登录
            if(packet->mode == 3){
        ...
        case CommandEnum_AddFriend://添加好友
            ...
            break;
        case CommandEnum_delFriend://删除好友
            break;
        case CommandEnum_PrivateChat://私聊
            handlePrivateChat(rMsg->body + sizeof(DeMessagePacket));
            break;
        case CommandEnum_CreateGroup://创建群组
            break;
        case CommandEnum_GetGroupList://获取组群列表
            getGroupList(rMsg->body + sizeof(DeMessagePacket));
            break;
        case CommandEnum_GetGroupInfo://获取群信息
            getGroupInfo(rMsg->body + sizeof(DeMessagePacket));
            break;
        case CommandEnum_GetFriendInfo://获取好友信息
...
{
    AddFriendDialog* addFriendDlg = new AddFriendDialog();//添加好友窗体

    addFriendDlg->show();
    if(addFriendDlg->exec() == QDialog::Accepted)//模态
    {
        AddFriendInfoReq addFriendInfoReq = addFriendDlg->getAddFriendInfoReq();
        addFriendInfoReq.m_senderAccount = m_userInfo.m_account;
        ChatLogInfo()<<u8"好友账号:"<<addFriendInfoReq.m_friendAccount;
